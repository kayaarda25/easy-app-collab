  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Migrated project: the cron jobs send the new project's publishable key.
        const expected = process.env.NEW_SUPABASE_PUBLISHABLE_KEY || process.env.SUPABASE_PUBLISHABLE_KEY;
